...
const getDataCollectionSubjects = (request, response) => {
  ...
  if(data_collection_id != undefined && !isNaN(data_collection_id)){
    pool.query('SELECT S.id, S.name, S.surname, S.contact, S.location, SR.creation_date, SR.status FROM survey_response SR, survey_component_response SCR, subject S WHERE SR.id = SCR.survey_response_id AND SR.subject_id = S.id AND SCR.user_id = $1 AND SR.status = \'incomplete\' AND survey_id IN (SELECT id FROM survey WHERE data_collection_id = $2)', 
