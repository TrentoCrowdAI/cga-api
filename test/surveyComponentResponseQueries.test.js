...
const dummyIncompleteComponentResponse = require('./dummies.js').dummyIncompleteComponentResponse;
...
describe('Test /componentResponses method root path', () => {
  ...
  test('Test PUT with incomplete date', (done) => {
    request(app).put('/componentResponses/' + dummyComponentResponse.id).set('Accept', /json/).send({survey_component_response: dummyIncompleteComponentResponse}).then((response) => {
      expect(response.statusCode).toBe(400);
      done();
    });
  });
  test('Test PUT with invalid id and incomplete data', (done) => {
    request(app).put('/componentResponses/' + invalid_id).set('Accept', /json/).send({survey_component_response: dummyIncompleteComponentResponse}).then((response) => {
      expect(response.statusCode).toBe(400);
      done();
    });
  });
  test('Test PUT with string id and incomplete data', (done) => {
    request(app).put('/componentResponses/' + string_id).set('Accept', /json/).send({survey_component_response: dummyIncompleteComponentResponse}).then((response) => {
      expect(response.statusCode).toBe(400);
      done();
    });
  });
